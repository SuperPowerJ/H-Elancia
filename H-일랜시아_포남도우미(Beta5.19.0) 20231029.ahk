...
Global EditList := ["힐링포션사용제한", "HP마을귀환사용제한", "MP마을귀환사용제한", "리메듐사용제한", "마나포션사용제한", "브렐사용제한", "식빵사용제한", "MP마을귀환사용여부", "넣을아이템","Multiplyer","NPC_MSG_ADR" ,"마지막사냥장소", "수련용길탐색딜레이","Weaponcheckdelay", "NPC대화딜레이"]
Global RadioButton := ["일무기", "일벗무바", "이무기", "이벗무바", "삼무기", "삼벗무바" ,"퍼펙트","일반","미스"]
Global CommonSkillList := ["대화","명상"]
Global NormalSkillList := ["더블어택","체력향상","민첩향상","활방어","마력향상","마법방어향상","회피"] ;집중 제외
Global TargetSkillList := ["훔치기","훔쳐보기","Sense","현혹","폭검","독침","무기공격"]
Global SkillListA := ["훔치기","훔쳐보기","Sense","현혹","폭검","독침","무기공격","더블어택","체력향상","민첩향상","활방어","마력향상","마법방어향상","집중","회피","대화","명상"]
...
스킬사용(스킬이름)
{
	...
	if (스킬번호 != 0)
	{
		...
		if( 스킬번호 != 등록된스킬번호)
		{
			sleep,10
			mem.write(0x0058D603, 스킬번호, "Char", aOffsets*)
			SB_SetText("스킬에러" 스킬번호 "!=" 등록된스킬번호,2)
...
행운장매수:
{
	...
	Loop,
	{
		if Coin!=1
			break
		행운장매수상태RunCount++
		guicontrol, ,행운장매수상태, %행운장매수상태RunCount%
		행깃교환Coin := 1
		SB_SetText("행깃구매시작",2)
		gui,submit,nohide
		맵번호 :=  mem.read(0x0058EB1C, "UInt", 0x10E)
		...
		if (행깃구매여부 == 1 && 맵번호 = 201)
		{
			...
			else if (mem.read(0x0058DAD4, "UInt", 0x178, 0xBE, 0x14) < 40)
			{
				행깃step := "행깃구매"
			}
			if (행깃step = "행깃교환")
			{
				...
				startinven := mem.read(0x0058DAD4, "UInt", 0x178, 0xBE, 0x14)
				행깃교환step := "행깃교환1"
				loop,
				{
					if Coin!=1
					...
					else if (행깃교환step = "행깃교환2")
					{
						마우스클릭(398,340)
						행깃교환step := "행깃교환3"
					}
					else if (행깃교환step = "행깃교환3")
					{
						sleep, 200
						if (행깃교환Coin!=1)
						{
						...
						NPCMsg := Check_NPCMsg()
						IfInString,NPCMsg,안녕하세요
						{
							KeyClick("k6")
						}
						else IfInString,NPCMsg,저는 그것을
						{
							temp:=get_NPCTalk_cordi()
							x:=temp.x - 13
							y:=temp.y + 15
						...
						else IfInString,NPCMsg,네! 그럼
						{
							KeyClick("k6")
							sleep, 200
						}
						else IfInString,NPCMsg,음... 어디보자
						{
							KeyClick("k6")
							sleep, 200
						}
						else IfInString,NPCMsg,풍선]을 얻다
						{
							행깃기록(NPCMsg)
							KeyClick("k6")
							loop,5
						...
						else IfInString,NPCMsg,가지고 오세요
						{
							KeyClick("k6")
							step := 0
							sleep, 300
				...
				loop, 2
				{
					if Coin!=1
						break
					NPC거래창첫번째메뉴클릭() ;키보드로 선택하기 위해 마우스로 첫번째 아이템 클릭
					sleep, 500
					repeat_to_write := 0
					target_to_write := mem.read(0x0045014C, "UInt", 0x20, 0x4C, 0x00, 0x30, 0x124) + 0x40C
				}
				Now_Selected := 1
				;I1:="행운의깃든종이"
				loop, 1
				{
					...
					if ( target = want )
					{
						mem.write(target_to_write, 5, "uint", aOffsets*)
						max_value := A_Index
					}
					target_to_write += 0x618
				}
				loop, %max_value%
				{
					if Coin!=1
				...
				sleep,10
				TouchCount:=0
				inven1 := mem.read(0x0058DAD4, "UInt", 0x178, 0xBE, 0x14)
				loop,
				{
					if Coin!=1
						break
					...
					sleep,100
					TouchCount++
					SB_SETTEXT( "구매시도중" TouchCount , 2)
					if (행깃교환Coin!=1)
					{
						break
					}
					inven2 := mem.read(0x0058DAD4, "UInt", 0x178, 0xBE, 0x14)
					if (Check_Shop("Buy")=0) || (inven2>=40)
					{
						break
					}
				}
...
{ ;스킬사용
	...
	for Index, spell in SpellList
	{
		...
		else if (temp_skill_usecheck == 1 && 몬스터리스트선택 != 0 && CurrentWeapon = 0) && (특수원거리타겟OID != 0x0 || 특수원거리타겟OID != "")
		{
	...
	for Index, spell in TargetSkillList
	{
		...
		gui,listview,몬스터리스트
		몬스터리스트선택 := LV_GetNext(0)
		if (temp_skill_usecheck == 1 && 몬스터리스트선택 != 0) && (spell = "무기공격" )
		{
			타겟스킬사용(spell, "클릭된대상")
		}
		else if (temp_skill_usecheck == 1 && 몬스터리스트선택 != 0) && (특수원거리타겟OID != 0x0 || 특수원거리타겟OID != "")
		{
...
{ ;무기자동바꾸기
	...
	else if (LastHit != CurrentHit)
	{
		...
		if (CurrentWeapon == 0 || UsePunch == 0)
		{
			...
			guicontrolget,Weaponcheckdelay
			sleep,%Weaponcheckdelay%
...
if (소각활성화 = 1)
{
	...
	if (소각딜레이 > 300 && 소각할아이템대기리스트갯수 > 0)
	{
		...
		if (아이템 != "" )
		{
			소각할아이템바꾸기 := mem.writeString(0x00590147, 아이템, "UTF-16", aOffsets*)
			메모리실행("하나씩소각")
			gui, listview, 소각할아이템대기리스트
			LV_Delete(1)
		}
...
if (은행넣기활성화 = 1)
{
	...
	if (소각딜레이 > 300 && 은행넣을아이템대기리스트갯수 > 0)
	{
		...
		if (아이템 != "" )
		{
			은행넣을아이템바꾸기 := mem.writeString(0x00590500, 아이템, "UTF-16", aOffsets*)
			메모리실행("은행넣기")
			LV_Delete(1)
		}
	}
...
Gui, Add, EDIT, x25 y230 w140 h20 vWeaponcheckdelay,
Gui, Add, EDIT, x25 y250 w140 h20 vNPC대화딜레이,
