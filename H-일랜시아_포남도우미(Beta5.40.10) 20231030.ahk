...
;수정일: 2023년 9월 23일
;버젼정보: Beta 5.40.10
; MultiThread 활용; 단, AHK는 MultiThread도 단일로 돌아간다.
; 1/1000초 단위로 쪼개져서 실행됨
; 예를 들어 1번 thread에서 sleep, 1 과 2번 thread 에서 sleep, 1 을 동시에 실행한다고 해도
; 1번 thread가 실행하고 그 다음에 2번 thread가 실행됨 (1990년대 x86방식)
;-----------------------------------------------------
...
Global 소각_DelayCount := A_tickCount
...
WriteLOG(Error)
{
	...
	newDataRow := 지금시각_T . "=" . Error  ; 시간=ERROR 형식으로 변경
...
IsMonster(Mapnumber,name)
{
	FileName := "MonsterList.ini"
	found := false
	IniRead, MonsterList, %FileName%, %Mapnumber%
	Monsters := StrSplit(MonsterList, "`n")
	for index, Monster in Monsters
	{
...
LoadCoordData()
{
	...
	;GuiControl, -Redraw, 좌표리스트

	IniRead, allMaps, %temp_저장위치%, %맵번호%
	MapList := StrSplit(allMaps, "`n")
	for index, coord in MapList
	{
		IniRead, existingData, %temp_저장위치%, %맵번호%, %A_index%
		fields := StrSplit(existingData, ",")
		fields[2] := A_index
		gui,listview,좌표리스트
		LV_Add("", fields*)
...
이동속도제어(Speed) ;
{
	if( mem.write(0x0058DAD4, Speed, "UInt", 0x178, 0x9C) != Speed)
		mem.write(0x0058DAD4, Speed, "UInt", 0x178, 0x9C) ;달리기걷기
	if( mem.write(0x0058DAD4, Speed, "UInt", 0x178, 0x98) != Speed)
		value := mem.write(0x0058DAD4, Speed, "UInt", 0x178, 0x98) ;걷기
	if( mem.write(0x0058DAD4, Speed, "UInt", 0x178, 0xa4) != Speed)
		value := mem.write(0x0058DAD4, Speed, "UInt", 0x178, 0xa4) ;헤엄치기
}
...
LoadCoordData()
SetTimer,메모리감시,50
SetTimer,스킬사용하기,2000
SetTimer,메모리_주변검색,330
SetTimer,접속여부확인,5000
SetTimer,기타동작,1000
SetTimer,무기자동바꾸기,%무기교체딜레이%
guicontrolget,시작시간
if (시작시간 = "시작시간")
{
...
else if (맵번호 = 4005)
{
	...
	{
		공격여부 := mem.read(0x0058DAD4, "UInt", 0x178, 0xEB)
		SB_SetText("공격여부" 공격여부 "자동사냥여부" 자동사냥여부,2)
		if (공격여부 != 0 && 자동사냥여부 = 1)
		{
			keyclick("Tab")
			sleep, 350
...
{ ;스킬사용
	...
	if (원거리대상갯수 > 0)
	{
		...
		;
		*/
		if(CurrentWeapon = 0 or CurrentWeapon =  "스태프")
		{
...
loop,2 ;이런식으로 영역에 {} 표기를 해놓으면 Loop 혹은 for 같은 조건을 추가 할 때 매우 편리
{ ;무기자동바꾸기
	;격투 0
	;시미터 4100
	;롱소드 4097
	...
	if (현재무기 != 0)
	{
		...
		else if (이번무기 == 현재무기 && WaaponLimit>=2)
		{
			이번무기수리필요 := 0
		}
		else if (삼번무기 == 현재무기 && WaaponLimit>=3)
		{
			삼번무기수리필요 := 0
		}
		else
	...
	*/
	...
	if (LastHit != CurrentHit)
	{
		...
		현재무기 := mem.read(0x0058DAD4, "UInt", 0x121)
		if (현재무기 = 일번무기 && WaaponLimit = 1)
		{
			다음무기 := 이번무기
		}
		else if (현재무기 = 이번무기 && WaaponLimit = 2)
		{
			다음무기 := 삼번무기
		}
		else if (현재무기 = 이번무기 && WaaponLimit > 2)
		...
		;SB_SetText(현재무기, 2)
		if (현재무기 == 0 || UsePunch == 0)
		{
			keyclick(WeaponNumber)
			sleep, 1
			/*
			loop, 2 ; 빠르지만 서버끊김 빈번 (서버: 1초 3무기 룰)
			{
				if coin != 1
					break
...
{ ;맵이 동일한지 확인
	...
	맵이름 := mem.readString(mem.getModuleBaseAddress("jelancia_core.dll")+0x44A28, 50, "UTF-16", 0xC)
	차원 := Get_Dimension()
	if (CHECKED맵번호 != 맵번호 && 맵번호 != "" && 맵번호 != 0)
	{
		;SB_SETtext("CHECKED맵번호" CHECKED맵번호 "현재맵번호" 맵번호 ,2)
		CHECKED맵번호 := 맵번호
		SB_SetText("맵변경감지대기" 전체대기,1)
		전체대기 := 3
...
for index, result in MonsterList
{
	...
	else
	{
		...
		kind := "알수없음"
		종류확인 := ["게임내고용상인들", "MonsterInfo", "게임내NPC들", "게임내쓸때없는존재들", "이름이바뀌는존재들"]
			...
			if (kind = "알수없음")
			{
				for index, PossibleSeller in %종류%
				{
					if (InStr(find_name, PossibleSeller))
					{
						if(종류 = "게임내고용상인들")
							kind := "고용상인"
						else if(종류 = "MonsterInfo")
							kind := "몬스터"
						else if(종류 = "게임내NPC들")
							kind := "NPC"
						else if(종류 = "게임내쓸때없는존재들")
							kind := "간판"
		...
		if (kind = "알수없음")
		{
			for index, PossibleMonster in MonsterInfo
			{
				IniRead, existingData, %FileName%, %맵번호%, %A_index%
				fields := StrSplit(existingData, ",")
				if (fields[1] = find_name)
				{
					kind := "몬스터"
					break
				}
		...
		if (kind = "알수없음")
		{
			for index, PossibleMonster in 게임내NPC들
			{
				if (InStr(find_name, PossibleSeller))
				{
					kind := "NPC"
...
for index, result in itemList
{
	...
	else
	{
		...
		loop % LV_GetCount()
		{
			LV_GetText(원하는아이템, 원하는아이템Count, 1)
			if (원하는아이템 != "" && find_name != "")
			{
				ifinstring, find_name, %원하는아이템%
				{
		...
		PassBlacklisted := True
		if (맵번호 = 3006 && find_x = 27 && find_y = 32 && find_z = 0 && IsWantedItem)
			PassBlacklisted := False

		if (IsNewOID && IsWantedItem && PassBlacklisted)
		{
			gui,listview,아이템리스트
			LV_Add("", "아이템", 차원, 맵이름, 맵번호, find_name, find_object_id, find_x, find_y, find_z, result, deletedelay)
		}
...
loop % LV_GetCount()
{
	...
	loop % LV_GetCount()
	{
		LV_GetText(원하는아이템이름1, 원하는아이템Count, 1)
		if (원하는아이템이름1 != "" && find_name != "") && instr(find_name, %원하는아이템이름1%)
		{
			IsWantedItem := True
			break
		}
	...
	;*/
	addr := mem.read(기존_주소, "UInt", aOffsets*)
	if (addr = 0x0053ECA4 && IsWantedItem = True)
	{
	}
	else if (addr != 0x0053ECA4 || IsWantedItem = False)
	{
		gui,listview,아이템리스트
		LV_delete(temp_i)
		temp_i -= 1
		...
		loop % LV_GetCount()
		{
			...
			if (블랙중복 = False)
			{
				...
				loop % LV_GetCount()
				{
					...
					if (P2Mdistance = 1)
						P2MExist++
					if (P2MExist > 2) ; 몹주변에 플레이어가 X명 초과(즉 X + 1명)로 있으면 바로 블랙등록, 일반사냥터는 매너겜을 위해 0 입력
					{
						SB_SetText("현재타겟 주변에 " P2MExist "명이상의 플레이어",2)
						gui,listview,블랙리스트
...
{
	SB_SETtext("ERROR-U-4",1)
	return
}

if !(인벤토리 > 0 && 인벤토리 <= 50)
{
	SB_SETtext("ERROR-U-3",1)
	return
}
...
if (자동사냥여부 = 1 && NPC대화창사용중 = 0)
{
	...
	if (맵번호 = 2 || 맵번호 = 1002 || 맵번호 = 2002 || 맵번호 = 3002 || 맵번호 = 4002)
	{
		...
		else if (마지막사냥장소 = 4005 && 맵번호 = 4002 && isMoving = 0)
		{
			호출할NPC := "리노아"
			리노아OID있는지여부확인 := CallNPC(호출할NPC)
			if(리노아OID있는지여부확인 = 1)
			{
				gosub, 리노아호출입장
				return
			}
			else if (리노아OID있는지여부확인 = 0)
			{
				SB_SetText("리노아호출실패",2)
				좌표입력(120,182,1)
				SB_SetText("자동이동10",4)
				메모리실행("좌표이동")
				return
...
If (CurrentTime = "23:58")  ; 포남링교환시간
{
	...
	if (맵번호 = 4002)
	{
		...
		Loop % LV_GetCount()
		{
			LV_GetText(NPC차원,A_index,2)
			LV_GetText(NPC이름,A_index,5)
			if (NPC차원 = 차원 && NPC이름 = "리노아" )
			{
				LV_GetText(리노아NPCOID,A_index,6)
			}
		}
		sleep, 100
		if(리노아NPCOID != "")
		{
			메모리권한변경및쓰기("NPC호출용1")
			메모리권한변경및쓰기("NPC호출용2")
			mem.write(0x00527b54, 리노아NPCOID, "UInt", aOffset*)
			메모리실행("NPC호출")
			SB_SETTEXT(리노아NPCOID " 호출" , "1")
		}
		gosub, 포남링교환
	}
}
...
{
	gui,listview,몬스터리스트
	자동사냥현재선택 := LV_GetNext(0)
	if (자동사냥현재선택 != 0)
	{
		lv_gettext(현재타겟분류,자동사냥현재선택,1)
		lv_gettext(현재타겟차원,자동사냥현재선택,2)
		lv_gettext(현재타겟맵이름,자동사냥현재선택,3)
		lv_gettext(현재타겟맵번호,자동사냥현재선택,4)
		lv_gettext(현재타겟이름,자동사냥현재선택,5)
		lv_gettext(현재타겟OID,자동사냥현재선택,6)
		lv_gettext(현재타겟X,자동사냥현재선택,7)
		lv_gettext(현재타겟Y,자동사냥현재선택,8)
		lv_gettext(현재타겟Z,자동사냥현재선택,9)
		mem.write(0x00590730, 현재타겟OID, "UInt", aOffsets*)
		mem.write(0x00584C2C, 현재타겟OID, "UInt", aOffsets*)
		guicontrol,,근거리타겟OID,%현재타겟OID%
		gui,listview,몬스터리스트
		...
		else ;블랙리스트에 등록할지 말지 결정
		{
			IsMoving := mem.read(0x0058EB1C, "UInt", 0x174)
			근접체크X := 현재타겟X - 좌표X
			근접체크Y := 현재타겟Y - 좌표Y
			if (시작타격번호 != 현재타격번호) && (근접체크X != 1 || 근접체크Y != 1)
			{
				MLS_delay := 0
			...
			else if ( 좌표X = 시작X && 좌표Y = 시작Y && 시작타격번호 = 현재타격번호 && MLS_delay > 1) && (근접체크X != 1 || 근접체크Y != 1)
			{
				KeyClick("AltR")
				gui,listview,몬스터리스트
				LV_Modify(0, "-Select")
				...
				if (블랙중복 = False)
				{
					LV_Add("",현재타겟분류,현재타겟차원,현재타겟맵이름,현재타겟맵번호,현재타겟이름,현재타겟OID,현재타겟X,현재타겟Y,현재타겟Z,6000)
				}
				StartCount := A_TickCount
			}
			else if (IsMoving != 0)
				...
				if(타겟_X > 0 && 타겟_Y > 0 && IsMoving = 0)
				{
					if(타겟_Z = 0 || 타겟_Z = 1)
					{
						...
						if ( 맵번호 = 237 || 맵번호 = 1403 || 맵번호 = 2300 || 맵번호 = 3300 || 맵번호 =  3301 || 맵번호 =  11)
						{
							...
							if ( 마지막메모리실행시간확인 > 1000 )
							{
								SB_SetText(줍줍할아이템 "발견",1)
								메모리실행("좌표이동")
								마지막메모리실행시간 := A_TickCount
							}
							return
...
gui,listview,좌표리스트
좌표리스트갯수 := LV_GetCount()
if (자동이동여부 = 1 && NPC대화창사용중 = 0 && 좌표리스트갯수>0 )
{
...
if (I_Delay > 1000)
{
	...
	loop, 50
	{
		...
		gui,listview,소지아이템리스트
		if (LV_GetCount() >= A_INdex)
		{
			lv_gettext(기존아이템,A_Index,2)
			lv_gettext(기존수량,A_Index,3)
			if (기존아이템 != invenitem || 기존수량 != ItemCount)
				LV_Modify(A_Index, "", a,invenitem,ItemCount)
		}
		else ;(invenitem != "" && invenitem != false )
...
{
;600x400
Gui, Add, Tab2, vTab1 x2 y2 w496 h416 AltSubmit cBlack gTab, 기본|설정|아템|이동|검색|파티|기타|개발용|스킬
Gui, Font, S9 Arial ,
Gui, Color, FFFFFF
gui, tab, 1
;캐릭터선택영역
...
Gui, Add, Text, x335 y320 w140 h20,식빵구매(라깃사용시 체크)
...
Gui, Add, checkbox, x260 y170 v몸통지르기사용, 몸찌
Gui, Add, checkbox, x260 y190 vRemoveArmor사용, 리뭅
...
Gui, Add, text, x125 y230 w100 h20 v시작체력 , 시작체력
Gui, Add, text, x125 y250 w100 h20 ,
Gui, Add, text, x125 y270 w100 h20 ,
Gui, Add, text, x125 y290 w100 h20 ,
Gui, Add, text, x125 y310 w100 h20 ,
...
Gui, Add, text, x25 y250 w100 h20  , 마을귀환이유
Gui, Add, text, x25 y250 w100 h20 ,
Gui, Add, text, x25 y270 w100 h20 ,
Gui, Add, text, x25 y290 w100 h20 ,
Gui, Add, text, x25 y310 w100 h20 ,
...
Gui, Add, EDIT, x395 y230 w70 h20 vMoveSpeed,
Gui, Add, EDIT, x395 y250 w70 h20 v게임배속,
