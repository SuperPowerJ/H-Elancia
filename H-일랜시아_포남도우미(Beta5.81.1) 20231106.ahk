...
;GUI용 이름들
RadioButton := ["주먹", "일무기", "이벗무바", "퍼펙트", "일반", "미스"]
...
현재모드_DDLOptions := ["대기모드","자동감응","일반자사","포남자사","포북자사","광물캐기","배달하기","행깃교환","행깃구매"]
...
;{
	...
	포프레스네나가기가능맵 := [4200,4207,4209,4211,4213,4215,4216,4219]
return
;}
...
골드바:
	...
	loop,
	{
		...
		else if (IsDataInList(맵번호, 포프레스네나가기가능맵) && 차원 = "알파") ;마을의 베이커리 ; 마법상점 ; 안이라면
		{
			gosub, 포프레스네상점나가기
			continue
		}
...
;{
	...
	loop,
	{
		...
		else if (IsDataInList(맵번호, 포프레스네나가기가능맵)) ;마을의 베이커리 ; 마법상점 ; 안이라면
		{
			gosub, 포프레스네상점나가기
			continue
		}
...
;{
;최소충족조건
if !(아이템갯수["GARLID"] > 1000000 && 아이템갯수["정령의보석"] > 10)
{
	그레이드필요 := False
	return
}
						...
						loop,
						{
							if (현재모드 = "포남자사" || 현재모드 = "포북자사") && (서버상태)
							{
								...
								if (무기수리필요 || 식빵구매필요 || 라깃구매필요 || 그레이드필요)
								{
									...
									else if (그레이드필요 && 아이템갯수["GARLID"] > 1000000 && 아이템갯수["정령의보석"] > 10)
									{
										gosub, 그레이드하기
										continue
									}
										...
										else if (근접체크 = 1 && 공격여부 != 0) ;도착했다면
										{
											...
											if (버스기사모드 = 0)
											{
												...
												Loop,
												{
													...
													상승어빌카운트 := mem.read(상승어빌주소 + 0x264 + 0x8, "UShort", aOffsets*)
													필요어빌카운트 := mem.read(상승어빌주소 + 0x264 + 0xA, "UShort", aOffsets*)
													Delay := A_TickCount - AttackStartCounter
													if (Delay > 3000)
													{
														AttackStartCounter := A_TickCount
														if (좀비여부 = 0)
													...
													else if (IsWordInList(상승어빌, 공격어빌))
													{
														...
														;SB_SetText("(" 사용할무기수량 ") Z:" 좀비여부 " W:" 무바여부, 1)
								...
								else if (좌표갯수 > 0)
								{
									...
									{
										MLS_delay++
										SB_SetText(좌표_현재선택 "번 좌표에 도달불가 " MLS_delay "/2",2)
										keyclick("AltR")
										sleep,1
										시작X := 좌표X
										시작Y := 좌표Y
...
Loop, %LVCount%
{
    ...
    ; Col12 값이 현재 가장 낮은 값보다 낮고, Col5 값이 WantedList에 포함되고, Col6 값이 BlackList에 없는 경우
    if (col11Value < 3 && col12Value < lowestCol12Value && !IsDataInList(col6Value, BlackList)) && (IsDataInList(col5Value, WantedItems) || WantedItemlength = "" || WantedItemlength < 1)
    {
        lowestCol12Value := col12Value
        selectedRow := thisRow
    }
