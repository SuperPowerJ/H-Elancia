...
Setting_Values:
	...
	Global AutoBuyInk := ["하얀색","분홍색"]

	Global AutoBuyPant := ["하얀색","분홍색"]
...
ShowGui:
	...
	Mode_kinds := ["상태체크","광물캐기","자사먹자","길탐수련","밥통작","겜섬오토" ] ; alpha version 1.0.0 7/1
	...
	Gui, Add, Tab2, +Theme vTab1 gTab1 x175 y2 w330 h315 , 길탐수련|단체활동|잠수체잠|자동구매|광물캐기|자사먹자|낚시먹자|길탐배달|일확천금|기타설정
	...
	gui, tab, 10 ;기타설정
		...
		Gui, Add, Text,  x%x_temp2% y%y_temp% w100 h20 , 스탭별 테스트
		...
		Gui, Add, Text,  x%x_temp3% y%y_temp% w120 h20 , 자동파티 파티장=1번
			y_base := y_temp1
			x_base := x_temp3
			x_base2 := x_base+20
			x_base3 := x_base2+90
			...
			Gui, Add, Button, x%x_base2% y%y_base% w84 h15 gparty, 파티맺기
	...
	gui, tab, 1 ;길탐수련
		...
		Gui, Add, Text, x%x_temp% y%y_temp6% w84 h20 ,대화 단축키:
		Gui, Add, Text, x%x_temp% y%y_temp7% w84 h20 ,명상 단축키:
		...
		gui, tab, 2 ; 단체활동
			...
			;Gui, add, text, x%temp_X0% y%temp_Y0%, 수리NPC이용
			...
			;Gui, add, Text, x%x_base% y%y_positionA% w%textw% h14 BackgroundTrans, 딜레이
			;variable_temp := Player%A%WaitRepair
			;Gui, Add, Edit, x%x_positionA% y%y_positionB% w%textw% h14 vPlayer%A%WaitRepair, %variable_temp%
			;Gui, Add, Text, x%x_positionB% y%y_positionB% w%textw% h14 , 초
...
Gui, Submit, Nohide
	...
	if(MapNumber=3204){
		loop,2 {
			...
			sleep, 200
			loop, 2 {
				PostClick(205,57) ;키보드로 선택하기 위해 마우스로 첫번째 아이템 클릭
			sleep, 200
			}
		Now_Selected := 1
		;카레푸메뉴
...
CharStatusCheck(){
		...
		Guicontrol,, CharLocation, 위치 : %Elancia_mapName%(%MapNumber%)
		Temp:=Get_HP()
		HPNow:=Temp.Now
		HPMax:=Temp.Max
		...
		Guicontrol,, Player%currentplayer%status,위치: %Elancia_mapName%(%MapNumber%) 인밴토리: %inven% / 50
	}
