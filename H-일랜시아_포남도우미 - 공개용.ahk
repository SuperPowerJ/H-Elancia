...
/*
Global ThisProgramVersion :=
Global NewVersion := GetVersion()
http := new CHttp()
url :=
response := JSON.Load(http.Get(url name "/version"))
version := response.version
if (ThisProgramVersion < NewVersion && AskUpdate = True)
{
	MsgBox, 4,, 최신버전이 존재합니다. `n다운로드 하시겠습니까?
    IfMsgBox Yes
	{
		name := this.name
		ahk_src := A_ScriptDir "\" name ".ahk"
		url :=
		Urldownloadtofile,%url%,%ahk_src%
		MsgBox, 4,, 최신버전을 다운로드 하였습니다. `n다시 실행하시겠습니까?
		IfMsgBox Yes
...
CurrentMode_DDLOptions := ["대기모드","자동감응","일반자사","포남자사","포북자사","마법잠수","광물캐기","배달하기"] ;,"보물지도교환","낚시먹자","행깃구매","리스무기구매"]
...
;{
	...
	GameIslandMacroText(TT){
		...
		Send, !m
		Sleep,500
		Macro := mem.read(0x0058EB64, "UInt", aOffsets*)
	...
	NPC거래창첫번째메뉴클릭_판매()
	{
		tempx := mem.read(0x0058EB48, "UInt",0x98) + 205 - 233
		tempy := mem.read(0x0058EB48, "UInt",0x9C) + 57 - 173
		MouseClick(tempx,tempy)
	}
	...
	;{ ; ListView, 배열, 혹은 매트릭스 관리용 함수;
		...
		GetMoreInformation()
		{
			...
			FAME := mem.read(0x0058DAD4, "UInt", 0x178, 0x47)
			GuiControl,, FAME, % FAME
...
마을 := "로랜시아"
맵번호 := mem.read(0x0058EB1C, "UInt", 0x10E)
thousandsDigit := Floor(mod(맵번호,10000)/1000)
if (차원결정유지 != 1)
{
Dimension := mem.read(0x0058EB1C, "UInt", 0x10A)
if(Dimension>20000)
...
if (맵번호 = 2 && 현재차원 = 목적차원)
{
	SB_SetText(마을 "에 도착" ,2)
	return
}
else if (IsDataInList(맵번호, 나가기가능맵) && thousandsDigit == 0 && 현재차원 = 목적차원) ;마을의 베이커리 ; 마법상점 ; 안이라면
{
	gosub, 상점나가기
	맵번호 := mem.read(0x0058EB1C, "UInt", 0x10E)
	if (맵번호 = 2)
...
else if (IsDataInList(맵번호, 나가기가능맵) && 아이템갯수["라스의깃"]<1) ;마을의 베이커리 ; 마법상점 ; 안이라면
{
	라깃구매필요 := True
	gosub, 상점나가기
	if (맵번호 = 2)
...
if(Dimension>20000)
	...
	현재차원 := 목적차원
if (맵번호 = 1002 && 현재차원 = 목적차원)
{
	SB_SetText(마을 "에 도착" ,2)
	return
}
else if (IsDataInList(맵번호, 나가기가능맵) && thousandsDigit == 1 && 현재차원 = 목적차원) ;마을의 베이커리 ; 마법상점 ; 안이라면
{
	gosub, 상점나가기
	맵번호 := mem.read(0x0058EB1C, "UInt", 0x10E)
	if (맵번호 = 1002)
...
if(Dimension>20000)
	...
	현재차원 := 목적차원
if (맵번호 = 2002 && 현재차원 = 목적차원)
{
	SB_SetText(마을 "에 도착" ,2)
	return
}
else if (IsDataInList(맵번호, 나가기가능맵) && thousandsDigit == 2 && 현재차원 = 목적차원) ;마을의 베이커리 ; 마법상점 ; 안이라면
{
	gosub, 상점나가기
	맵번호 := mem.read(0x0058EB1C, "UInt", 0x10E)
	if (맵번호 = 2002)
...
if(Dimension>20000)
	...
	현재차원 := 목적차원
if (맵번호 = 3002 && 현재차원 = 목적차원)
{
	SB_SetText(마을 "에 도착" ,2)
	return
}
else if (IsDataInList(맵번호, 나가기가능맵) && thousandsDigit == 3 && 현재차원 = 목적차원) ;마을의 베이커리 ; 마법상점 ; 안이라면
{
	gosub, 상점나가기
	맵번호 := mem.read(0x0058EB1C, "UInt", 0x10E)
	if (맵번호 = 3002)
...
if(Dimension>20000)
	...
	현재차원 := 목적차원
if (맵번호 = 4002 && 현재차원 = 목적차원)
{
	SB_SetText(마을 "에 도착" ,2)
	return
}
else if (IsDataInList(맵번호, 나가기가능맵) && thousandsDigit == 4 && 현재차원 = 목적차원) ;마을의 베이커리 ; 마법상점 ; 안이라면
{
	gosub, 상점나가기
	맵번호 := mem.read(0x0058EB1C, "UInt", 0x10E)
	if (맵번호 = 4002)
...
;{
	...
	loop,
	{
		...
		loop, 5
		{
			...
			sleep,1000
			gosub, 아이템읽어오기
			if (아이템갯수["라스의깃"] >= 20) && ((아이템갯수["오란의깃"] >= 100 && 오란의깃사용여부 = 1) || 오란의깃사용여부 = 0)
			{
				거래창사용중 := False
				라깃구매필요 := False
				break
...
;{
	...
	loop,
	{
		...
		else if (맵번호 = 3014 || 맵번호 = 3015)
		{
			...
			gosub, 크로노시스가기
		IsMoving := mem.read(0x0058EB1C, "UInt", 0x174)
		if (isMoving = 0 || 좌표용맵번호 != 맵번호)
		{
			좌표입력(목표좌표X,목표좌표Y,목표좌표Z)
			좌표용맵번호 := 맵번호
			sleep, 1
			RunMemory("좌표이동")
			SB_SetText("이동 X,Y: " 목표좌표X "," 목표좌표Y, 2)
		}
	}
	return
;}
...
;{
	...
	loop,
	{
		...
		else if (맵번호 = 3504) ;만약 지도가없어 미로로 간다면
		{
			목표좌표X := 4
			목표좌표Y := 4
			목표좌표Z := 0
...
;{
	loop,
	{
		...
		if (isMoving = 0)
		{
			SB_SetText("탈출X,Y: " 목적지퇴장X "," 목적지퇴장Y, 2)
...
else if (맵번호 = 4005)
{
	...
	else if (한턴쉴까 = False)
	{
		...
		loop, 5
		{
			sleep,1000
			NPCMsg := mem.readString(NPC_MSG_ADR, 52, "UTF-16", aOffsets*)
			IsFormExist := mem.read(0x0058EB5C, "UInt", aOffsets*)
			FormNumber := mem.read(0x0058DAD0, "UInt", 0xC, 0x10, 0x8, 0xA0)
...
loop,
{
	...
	IfInString,NPCMsg,올랐습니다
	{
		...
		if (그레이드알림설정)
		{
			TMessage := TargetTitle . "의 " . 그레이드할어빌 . " 그레이드진행완료(" . 그레이드할어빌의그레이드 . "글 -> " . tt . "글)"
			텔레그램메시지보내기(TMessage)
		}
		mem.writeString(NPC_MSG_ADR, "", "UTF-16", aOffsets*)
		keyClick("K6")
...
;{
	...
	loop,30
	{
		...
		%그레이드% :=  mem.read(0x0058DAD4, "UInt", 0x178, 0xc2, 0x8, A, 0x8, 0x430)
		마법레벨 := "마법" . A_index
		Ability_name := %마법이름%
		Ability := %마법레벨%
		Ability_Grade := %그레이드%
		...
		if (Ability = 100)
		{
			...
			if (그레이드여부 = 1 && Ability_Grade >= 0 && Ability_Grade < 30)
			{
				그레이드필요 := True
				그레이드종류 := "마법" ; "어빌" || "마법"
				그레이드할어빌 := Ability_name
				그레이드할어빌의그레이드 := Ability_Grade
		...
		if(마법이름 != Fail && 마법이름 != "")
			LV_Add("", "마법", A_index, Ability_name, %그레이드%, Ability)
		else
			break
	}
	return
...
Gui, Add, Text, x345 y138 w60 h18 ,FAME
Gui, Add, Text, +Right x410 y138 w60 h18 vFAME,
...
Gui, Add, DropDownList, x280 y215 w80 vCurrentMode,대기모드||자동감응|일반자사|포남자사|포북자사|광물캐기|배달하기|마법잠수|낚시먹자
...
Y_coord += 22
Gui, Add, Button, x215 y%Y_coord% h20 w70 g로랜시아가기, 로랜시아
Gui, Add, Button, x290 y%Y_coord% h20 w60 g로랜시아퍼브가기,퍼브
Gui, Add, Button, x355 y%Y_coord% h20 w60 g로랜시아우체국가기,우체국
Gui, Add, Button, x420 y%Y_coord% h20 w60 g로랜시아목공소가기, 목공소

Y_coord += 22
Gui, Add, Button, x215 y%Y_coord% h20 w70 g에필로리아가기, 에필로리아
Gui, Add, Button, x290 y%Y_coord% h20 w60 g에필로리아퍼브가기, 퍼브
Gui, Add, Button, x355 y%Y_coord% h20 w60 g에필로리아우체국가기, 우체국
Gui, Add, Button, x420 y%Y_coord% h20 w60 g에필로리아목공소가기, 목공소

Y_coord += 22
Gui, Add, Button, x215 y%Y_coord% h20 w70 g세르니카가기, 세르니카
Gui, Add, Button, x290 y%Y_coord% h20 w60 g세르니카퍼브가기, 퍼브
Gui, Add, Button, x355 y%Y_coord% h20 w60 g세르니카우체국가기, 우체국
Gui, Add, Button, x420 y%Y_coord% h20 w60 g세르니카목공소가기, 목공소

Y_coord += 22
Gui, Add, Button, x215 y%Y_coord% h20 w70 g크로노시스가기, 크로노시스
Gui, Add, Button, x290 y%Y_coord% h20 w60 g크로노시스베이커리가기, 베이커리
Gui, Add, Button, x355 y%Y_coord% h20 w60 g크로노시스마법상점가기, 마법상점
Gui, Add, Button, x420 y%Y_coord% h20 w60 g크로노시스무기상점가기, 무기상점

Y_coord += 22
Gui, Add, Button, x215 y%Y_coord% h20 w70 g포프레스네가기, 포프레스네
Gui, Add, Button, x290 y%Y_coord% h20 w60 g포프레스네베이커리가기, 베이커리
Gui, Add, Button, x355 y%Y_coord% h20 w60 g포프레스네마법상점가기, 마법상점
Gui, Add, Button, x420 y%Y_coord% h20 w60 g포프레스네무기상점가기, 무기상점

Y_coord += 22
Gui, Add, Button, x215 y%Y_coord% h20 w70 g이동중지, 이동중지
Gui, Add, Button, x290 y%Y_coord% h20 w60 g감옥섬가기, 감옥섬
Gui, Add, Button, x355 y%Y_coord% h20 w60 g보물섬가기, 보물섬
Gui, Add, Button, x420 y%Y_coord% h20 w60 g세르니카편집국가기, 편집국
...
Gui, Add, text, x15 y370 w180 h15 v플레이어수,
Gui, Add, text, x15 y390 w180 h15 v몬스터수,
Gui, Add, text, x15 y410 w180 h15 v아이템수,
/*
...
loop, 72
{
	...
	gui, add, text, x%x_coord__% y%y_coord% w40 h15  v%temp_name%,
	y_coord += 20
		...
		if (서버상태)
		{
			...
			loop,
			{
				...
				else if (서버상태) ;만약 서버접속중이 확인되면 현재 모드를 확인
				{
					...
					else if (CurrentMode = "낚시먹자")
					{
						loop,
						{
							if (CurrentMode = "낚시먹자") && (서버상태) && (Coin)
							{
								IsMoving := mem.read(0x0058EB1C, "UInt", 0x174)
								if (IsMoving != 0)
								{
								...
								{
									if (골드바판매여부 && GALRID < 1000000 && 아이템갯수["골드바"] > 0)
									{
										gosub, 골드바팔기
										continue
									}
					...
					else if (CurrentMode = "마법잠수")
					{
						...
						loop,
						{
							if (CurrentMode = "마법잠수") && (서버상태) && (Coin)
							{
								...
								else if (MZ_Delay > 600000 || 현재FP <1 ) && (회복 = 1) && (서버상태) && Coin
								{
									SetTimer,스킬사용하기,off
									sleep, 1500
									AttackStartCounter := A_TickCount
								...
								if (수리 = 1 || 소야수리 = 1)  && (서버상태) && (MZ_Delay > 1000) && Coin
								{
									AttackStartCounter := A_TickCount
									gosub, 아이템읽어오기
...
if ((RM_Delay > 5000) || (자동사냥여부 = 1) || (CurrentMode = "낚시먹자"))
{
...
temp_c := LV_GetCount()
guicontrol,,플레이어수,플레이어: %temp_c% 명
return
;}
...
HighLevelFishes := ["전구어","사각어","더듬이","해파리","황금붕어","골치","래비쉬"]
HLF_Itemlength := HighLevelFishes.MaxIndex()
if ((LVSelect != 0) || (LVCount = 0))
{
	if (!IsDataInList_Multi(col5Value, HighLevelFishes) && HLF_Itemlength >= 1)
	{
		gui, listview, 아이템리스트
		LV_Modify(0,"-Select")
	}
...
HLF_Itemlength := 0 ; 기본값을 0으로 설정
if HighLevelFishes.MaxIndex() ; 배열이 비어있지 않은 경우
    HLF_Itemlength := HighLevelFishes.MaxIndex()
...
Loop, %LVCount%
{
    ...
    ; Col12 값이 현재 가장 낮은 값보다 낮고, Col5 값이 WantedList에 포함되고, Col6 값이 BlackList에 없는 경우
    if (col11Value < 3 && col12Value < lowestCol12Value && !IsDataInList_Multi(col6Value, BlackList)) && (IsDataInList_Multi(col5Value, HighLevelFishes) || HLF_Itemlength < 1)
    {
        lowestCol12Value := col12Value
        selectedRow := thisRow
    }
